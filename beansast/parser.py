...
class Parser:
    ...
    Instances should implement the parse method, in order to
    return an AST that was parsed by the given grammar.\

    p = Parser(grammar[, gmrfile])
    # provide a grammar to the constructor
    #  and, eventually, the name of the file of the 
    p(input[, inputfile])
    """)
...
class EarleyParser(Parser):
    __doc__ = textwrap.dedent("""\
    Parser which uses the Earley parsing algorithm.
    """)
    def _get_reversed_S(self):
        ...
        for i, state_set in enumerate(S):
            for item in state_set:
                if item.dot < len(self.grammar[item.rule_id]):
                    continue
                j = item.start
    ...
    def _is_good_item(self, item, size):
        ...
        return rule.name in self.grammar.axioms and item.end == size
    ...
    def parse(self):
        ...
        for item in S[0]:
            if self._is_good_item(item, len(S)-1):
                return self.search_item(0, item, S, self.grammar, self.input)
        else:
            # Handling error
...
class ASTBuilder:
    def __init__(self, parser=EarleyParser, lexer_grammar=LEXER_GRAMMAR, parser_grammar=PARSER_GRAMMAR):
        with open(lexer_grammar) as f:
            self.lexer_grammar = f.read()
        with open(parser_grammar) as f:
            self.parser_grammar = f.read()
        self.lexer = Lexer(self.lexer_grammar, file=lexer_grammar)
        self.parser = parser(self.parser_grammar, gmrfile=parser_grammar)
