...
pub fn regex_email(criterion: &mut Criterion) {
    ...
    // Start benchmark
    group.bench_function("Regex email: beans engine", |b| b.iter(|| beans_engine.find(&content)));
    group.bench_function("Regex email: std engine", |b| b.iter(|| std_engine.captures(&content)));
    group.finish();
}
