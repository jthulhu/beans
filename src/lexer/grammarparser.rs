...
mod tests {
	...
	let mut stream =
	    StringStream::new(String::from("whatever"), String::from("ignore"));
        assert_eq!(stream.pos(), 0);
        assert_eq!(LexerGrammarBuilder::read_keyword(&mut stream, "something"), false);
        assert_eq!(stream.pos(), 0);
        assert_eq!(LexerGrammarBuilder::read_keyword(&mut stream, "ignore"), true);
        assert_eq!(stream.pos(), 6);
    }
    #[test]
    fn grammar_parser_read_id() {
        let mut stream =
            StringStream::new(String::from("whatever"), String::from("to del"));
        assert_eq!(stream.pos(), 0);
        assert_eq!(LexerGrammarBuilder::read_id(&mut stream), Ok(String::from("to")));
        assert_eq!(stream.pos(), 2);
        LexerGrammarBuilder::ignore_blank(&mut stream);
        assert_eq!(stream.pos(), 3);
        assert_eq!(LexerGrammarBuilder::read_id(&mut stream), Ok(String::from("del")));
        assert_eq!(stream.pos(), 6);
        assert_eq!(
